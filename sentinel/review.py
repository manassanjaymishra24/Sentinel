...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Review, approve, and optionally execute a Sentinel response plan."
    )
    parser.add_argument("plan", type=Path, help="Path to a response plan JSON file.")
    parser.add_argument(
        "--output",
        type=Path,
        help="Where to write the reviewed plan. Defaults to updating the input file.",
    )
    parser.add_argument(
        "--approve-all",
        action="store_true",
        help="Approve every human-gated step without prompting.",
    )
    parser.add_argument(
        "--reject-all", action="store_true", help="Reject every human-gated step without prompting."
    )
    parser.add_argument(
        "--execute", action="store_true", help="Execute approved steps after review."
    )
    parser.add_argument(
        "--allow-execute",
        action="store_true",
        help="Required with --execute to run local response commands.",
    )
    parser.add_argument(
        "--forensics-dir",
        type=Path,
        default=Path("sentinel_data/forensics"),
        help="Directory for approved forensics snapshots.",
    )
    parser.add_argument(
        "--incident-db",
        type=Path,
        help="Optional SQLite database path for recording the review outcome.",
    )
    parser.add_argument(
        "--actor", default="local-analyst", help="Reviewer name stored in the incident database."
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
        help="Do not prompt. Requires --approve-all or --reject-all.",
    )
    return parser
...
def main() -> None:
    ...
    if args.incident_db:
        ...
        IncidentStore(args.incident_db).record_approval(
            reviewed.decision_id, outcome, actor=args.actor, details=reviewed.to_dict()
        )
        print(f"Recorded review outcome in {args.incident_db}")

    if args.execute:
        results = ResponseExecutor(forensics_dir=args.forensics_dir).execute_plan(
            reviewed, allow_execute=args.allow_execute
        )
