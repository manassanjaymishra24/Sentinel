...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source", choices=["windows"], default="windows", help="Live telemetry source.")
    parser.add_argument("--logs", nargs="*", default=DEFAULT_LOGS, help="Windows Event Log names to read.")
    parser.add_argument("--since-minutes", type=int, default=5, help="Lookback window per polling cycle.")
    parser.add_argument("--max-events", type=int, default=50, help="Maximum events per log per polling cycle.")
    parser.add_argument("--interval", type=int, default=30, help="Seconds between polling cycles.")
    parser.add_argument("--once", action="store_true", help="Run one polling cycle and exit.")
    parser.add_argument("--json-only", action="store_true", help="Print machine-readable reasoning JSON only.")
    parser.add_argument("--no-response-plan", action="store_true", help="Do not print the dry-run response plan.")
    parser.add_argument("--report-dir", type=Path, help="Optional directory to write markdown incident reports.")
    parser.add_argument("--response-dir", type=Path, help="Optional directory to write response plan JSON files.")
...
def run_cycle(args: argparse.Namespace) -> int:
    reader = WindowsEventLogReader(log_names=args.logs, since_minutes=args.since_minutes, max_events=args.max_events)
